...
import { LoginResponse, ProfileOwnResponse, ResponseUtils } from '../http/responses';
...
export class UserStateService {
  ...
  private last_action: Observable<string>;
  private last_action_details: Observable<any>;
  ...
  public log_out(): Observable<string> {
    console.log("====== LOG OUT!!! ");
  ...
  public update_profile(email: string, first_name: string, last_name: string): Observable<string> {
    const token = this.get_current_token();
    if ((token == undefined) || (token == null) || (token == "")) {
      this.set_last_action("TOKEN_ERROR_NOT_LOGGED");
      return this.get_last_action();
      // return of("TOKEN_ERROR_NOT_LOGGED");
    } 
    ...
    if (this.is_fake_mode) {
      ...
      this.authServiceReal.profile_own_update(token, email, first_name, last_name).subscribe({
        next: (resp) => {
          ...
          const ownResp = resp.body?.data as ProfileOwnResponse;
          console.log("ownResp name ", ownResp.constructor.name);
          this._set_current_user(ProfileOwnResponse.get_user_from_response_body(resp.body?.data));

          
          console.log("XXXX Current user ", this.get_current_user());
          // return this.get_last_action(); // ===========================
          // return of ("PROFILE_UPDATE_OK");
          this.set_last_action("PROFILE_UPDATE_OK");
        },
        error: (resp) => {
          ...
          if (ResponseUtils.resp_error_email_already_taken(resp)) {
            ...
            // return of("PROFILE_UPDATE_ERROR_ALREADY_EXISTS");
          } else {
            this.set_last_action("PROFILE_UPDATE_ERROR");
            // return of("PROFILE_UPDATE_ERROR");
          }
        },
        complete: () => { }
      });
  ...
  public update_password(current_password_to_check: string, new_password: string): Observable<string> {
    const token = this.get_current_token();
    if ((token == undefined) || (token == null) || (token == "")) {
      this.set_last_action("TOKEN_ERROR_NOT_LOGGED");
      // return this.get_last_action();
  ...
  public log_in(email: string, password: string): Observable<string> {
    if (this.is_fake_mode) {
      ...
      console.log("login-2 - REAL AUTH!!!");
      this.authServiceReal.log_in(email, password).
        subscribe({
          next: (resp: HttpResponse<DataResponse<LoginResponse>>) => {
            this._set_is_logged(true);
            this.is_logged_subb.next(true);
            console.log("Q@QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ RESP, ", resp, resp.body, resp.body?.data);
            console.log("Q@QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ RESP BODY CONSTRUCTOR, ", resp.body?.constructor);
            console.log("Q@QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ RESP BODY DATA CONSTRUCTOR, ", resp.body?.data?.constructor);
            // return;
            const curUser = UserModel.from_login_response(resp.body?.data);
            ...
            this._set_current_token(((resp.body?.data != undefined) && (resp.body?.data != null)) ? resp.body?.data.token : null);
  ...
  public register(email: string, password: string, first_name: string, last_name: string): Observable<string> {
    if (this.is_fake_mode) {
  ...
  public confirm_registration(email: string, register_confirm_token: string): Observable<string> {
    if (this.is_fake_mode) {
  ...
  public send_reset_password_request(email: string): Observable<string> {
    if (this.is_fake_mode) {
  ...
  public confirm_password_reset(email: string, password_reset_token: string, new_password: string): Observable<string> {
    if (this.is_fake_mode) {
